...
export const renderLinePanningWithUpdatedLegend = (id) => {
    ...
    const graphDataY2 = utils.deepClone(
        getDemoData(`#${id}`, "LINE_TIMESERIES_DATELINE").data[2]
    );
    const graphDataH = {
        ...graphData,
        values: [],
    ...
    const createGraph = () => {
        ...
        if (graphDataH.values === graphData.values) {
            graph.reflow({
                ...graphDataY,
                yLabel: "Data Label 1, Data Label 2"
            });
            graph.reflow(graphDataY2);
            graphData = graphDataY;
        } else {
            graph.reflow({ ...graphDataH, yLabel: " " });
            graph.reflow({ ...graphDataY2, values: [] });
    ...
    const graph = Carbon.api.graph(axisData);
    graph.loadContent(Carbon.api.line(graphData));
    graph.loadContent(Carbon.api.line({ ...graphDataY2, values: [] }));
    axisData.axis = graph.config.axis;
